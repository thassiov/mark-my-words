...
import type { Record, SelectionInput } from '../shared/types.js';
import type { Repository } from '../storage/repository.js';

import { RecordService, normalizeTags } from './record-service.js';
...
class InMemoryRepo implements Repository<Record> {
...
describe('RecordService', () => {
  ...
  describe('list', () => {
    ...
    it('returns items sorted newest-first', async () => {
      ...
      const a = await service.saveSelection({ ...baseInput, selectedText: 'a' });
      vi.setSystemTime(new Date('2026-05-04T11:00:00Z'));
      const b = await service.saveSelection({ ...baseInput, selectedText: 'b' });
      vi.setSystemTime(new Date('2026-05-04T12:00:00Z'));
      const c = await service.saveSelection({ ...baseInput, selectedText: 'c' });
    ...
    it('respects limit', async () => {
      for (let i = 0; i < 5; i++) {
        ...
        await service.saveSelection({ ...baseInput, selectedText: `item ${String(i)}` });
      }
      const result = await service.list({ limit: 2 });
      expect(result).toHaveLength(2);
      expect(result.map((s) => (s.type === 'selection' ? s.selectedText : ''))).toEqual([
        'item 4',
        'item 3',
      ]);
    });
  ...
  describe('delete', () => {
    ...
    it('reduces count by 1', async () => {
      const a = await service.saveSelection(baseInput);
      await service.saveSelection(baseInput);
  ...
  describe('archive / unarchive', () => {
    ...
    it('throws when archiving an unknown id', async () => {
      await expect(service.archive('no-such-id')).rejects.toThrow('Record no-such-id not found');
    });

    it('throws when unarchiving an unknown id', async () => {
      await expect(service.unarchive('no-such-id')).rejects.toThrow('Record no-such-id not found');
    });
  });
  ...
  describe('list with archived filter', () => {
    ...
    it('archived: false matches default', async () => {
      const a = await service.saveSelection({ ...baseInput, selectedText: 'a' });
      await service.saveSelection({ ...baseInput, selectedText: 'b' });
      await service.archive(a.id);

      expect(
        (await service.list({ archived: false })).map((s) =>
          s.type === 'selection' ? s.selectedText : '',
        ),
      ).toEqual(['b']);
    });
    ...
    it('archived: true returns only archived snippets sorted by archivedAt desc', async () => {
      ...
      expect(result.map((s) => (s.type === 'selection' ? s.selectedText : ''))).toEqual(['a', 'b']);
    });

    it('archived: true ignores active snippets', async () => {
      await service.saveSelection({ ...baseInput, selectedText: 'active' });
      const archived = await service.saveSelection({ ...baseInput, selectedText: 'archived' });
  ...
  describe('tags', () => {
    it('save normalizes tags: lowercases, trims, dedupes, drops empty', async () => {
      const snippet = await service.saveSelection({
        ...baseInput,
    ...
    it('save with all-empty tags omits the field', async () => {
      const snippet = await service.saveSelection({ ...baseInput, tags: ['  ', ''] });
    ...
    it('update can replace existing tags', async () => {
      const snippet = await service.saveSelection({ ...baseInput, tags: ['old'] });
    ...
    it('update with empty tags array clears the field', async () => {
      const snippet = await service.saveSelection({ ...baseInput, tags: ['will', 'be', 'gone'] });
    ...
    it('update with explicit undefined clears the field', async () => {
      const snippet = await service.saveSelection({ ...baseInput, tags: ['gone'] });
